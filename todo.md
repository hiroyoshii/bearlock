...
2026-09-04時点の残タスク。TestFlight経由の実機インストール、Family Controls Distribution承認、CIでのbuild / mock e2e / screenshot取得までは完了済み。Version 1.0はIAPなしで提出し、課金対応は次版以降へ延期する。
...
- [x] 保存済み対象リストで、アプリ名・カテゴリ名・Webサイト名が濃色で読めることを確認する。
...
- [x] Shield画面のタイトルが、対象アプリ名を主語にして表示されることを確認する。
...
- [x] 日本語環境で曜日ボタンが `日 月 火 水 木 金 土` 表示になることを確認する。
...
- [x] Version 1.0から任意サポートIAPを外し、次版以降へ延期する。
  - アプリ内導線、StoreKit実装、ローカライズ、公開中のPrivacy Policy / SupportページからIAP記述を削除する。
  - App Store ConnectのIAP商品は「提出準備中」のまま保持し、Version 1.0の提出物には追加しない。
...
- [ ] Apple Developer Programの登録住所変更が反映されるのを待つ。
  - Version 1.0の無料リリースはこの住所変更を待たずに進める。
  - 反映後、W-8BENとU.S. Certificate of Foreign Status of Beneficial Ownerを提出する。
  - 有料アプリ契約、税務情報、銀行口座が有効になったことを確認する。
  - IAPを再実装した新バージョンで、TestFlightの商品取得と実機購入を確認する。
...
- [x] v0.2以降の任意サポートを検討する。
  ...
  - Version 1.0から画面とStoreKit実装は削除済み。再導入時はGit履歴と調査資料を参照する。
